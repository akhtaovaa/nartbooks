...
function redirectToAuth() {
    const currentUrl = window.location.pathname.split('/').pop() || 'admin.html';
...
async function initAdmin() {
    try {
        ...
        if (!window.api) {
            console.error('API модуль не загружен. Проверьте, что api.js загружается перед admin.js');
            showAccessDenied('Ошибка: API модуль не загружен. Обновите страницу.');
            return;
        }
